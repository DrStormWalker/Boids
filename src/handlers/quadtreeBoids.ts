...
export class QuadTreeBoids extends BoidHandler {
    numBoids = 200;
    visualRange = 75;
    boids: Boid[] = [];
    boidTree: QuadTree<number>;

    constructor(p: p5, width: number, height: number) {
        super(width, height);

        this.boidTree = new QuadTree<number>({
        ...
        for (let i = 0; i < this.numBoids; i++) {
            ...
            const index = this.boids.push(boid) - 1;

            this.boidTree.insert({
                pos: boid.pos.copy(),
                width: 0,
                height: 0,
            }, index);
    ...
    updateBoids(p: p5): void {
        for (let boid of this.boids) {
            const visible = this.boidTree.retrieve(createRect(
                boid.pos.copy().sub(this.visualRange, this.visualRange),
                this.visualRange * 2,
                this.visualRange * 2,
            )).map(value => (this.boids)[value[1]]);

            console.log(visible.length);

            const pos = boid.pos.copy().sub(this.visualRange, this.visualRange)
